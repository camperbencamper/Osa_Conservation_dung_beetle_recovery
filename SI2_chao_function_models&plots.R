...
p_dissim <- vegdist(trait_df_ordi[,names(trait_df_ordi) %in% functions], method="chao", binary=FALSE)
...
j_dissim <- vegdist(trait_df_ordi[,names(trait_df_ordi) %in% functions], method="chao", binary=TRUE)
...
#plotting SI II: Fig. S2, panels C and D

#pull p1 (A) and p2 (B) from taxonomy model script: 'DB_chao_taxonomy_models&plots.R"

#abundance
p3 <- ggplot(data=p_sim, aes(x=year, y=similarity, colour=treatment)) + ylab("Chao-Jaccard Similarity (Abundance)") + xlab("Year") + ggtitle("C)") +
...
#incidence
p4 <- ggplot(data=j_sim, aes(x=year, y=similarity, colour=treatment)) + ylab("Chao-Jaccard Similarity (Incidence)") + xlab("Year") + ggtitle("D)") +
  geom_boxplot(position = position_dodge(width = 0.9), size=1.75, outlier.size=4) + theme_bw(40) + theme(legend.position = "none") + ylim(0, 0.9) +
  stat_summary(fun = median, geom = 'line', linewidth=1.75, aes(group = treatment),position = position_dodge(width = 0.9))+
  scale_color_manual(breaks = c("N","L","M","H"), values=c("red2", "dodgerblue","chartreuse3","darkorchid3"))
...
grid_layout <- plot_grid(p1, p2, p3, p4, align = "v", nrow = 2, rel_heights = c(1/4, 1/4, 1/4), rel_widths = c(1/4, 1/4, 1/4))

ggsave("./FigureS2_chao.png", grid_layout, height = 20, width = 30)
