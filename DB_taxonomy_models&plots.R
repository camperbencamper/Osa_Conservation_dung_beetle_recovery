...
bc_bet2 <- glmmTMB(similarity ~ year,#BEST MODEL; -379.68
                   family = ordbeta(),
                   start=list(psi = c(-1, 1)),
                   data = p_sim)
summary(bc_bet2)
#pairs(b_posthoc2, adjust = "bonf") #https://stats.stackexchange.com/questions/9425/bonferroni-or-tukey-when-does-the-number-of-comparisons-become-large
...
j_dissim <- vegdist(comm_data_ordi[,names(comm_data_ordi) %in% DB_species], method="jaccard")
...
j_bet2 <- glmmTMB(similarity ~ year, # BEST MODEL; -493.22
                  family = ordbeta(),
                  start=list(psi = c(-1, 1)),
                  data = j_sim)
summary(j_bet2)
...
#full figure printed (including panels C and D) in "DB_function_models&plots.R"


#bray-curtis
p1 <- ggplot(data=p_sim, aes(x=year, y=similarity, colour=treatment)) + ylab("Bray-Curtis Similarity") + xlab("Year") + ggtitle("A)") +
  geom_boxplot(position = position_dodge(width = 0.9), size=1.75, outlier.size=4) + theme_bw(40) + theme(legend.position = "none") + ylim(0, 0.18) +
...
#jaccard
p2 <- ggplot(data=j_sim, aes(x=year, y=similarity, colour=treatment)) + ylab("Jaccard Similarity") + xlab("Year") + ggtitle("B)") +
  geom_boxplot(position = position_dodge(width = 0.9), size=1.75, outlier.size=4) + theme_bw(40) + theme(legend.position = "none") + ylim(0, 0.18) +
  stat_summary(fun = median, geom = 'line', linewidth=1.75, aes(group = treatment),position = position_dodge(width = 0.9))+
  scale_color_manual(breaks = c("N","L","M","H"), values=c("red2", "dodgerblue","chartreuse3","darkorchid3"))
...
#full figure printed (including panel C) in "DB_function_models&plots.R"


#richness
p1 <- ggplot() + theme_bw(40) + ylab("Species Richness") + xlab("Year") + ggtitle("A)") + theme(legend.position = "none") +
  geom_boxplot(data=comm_data[comm_data$treatment!="P",], size=1.75, outlier.size=4, aes(x=year, y=richness, colour=treatment)) +
  geom_boxplot(data=comm_data[comm_data$treatment=="P",], size=1.75, outlier.size=4, aes(x=year, y=richness, colour=treatment)) +
  scale_color_manual(breaks = c("N","L","M","H","P"), values=c("red2", "dodgerblue","chartreuse3","darkorchid3","darkred"))

#abundance
p2 <- ggplot() + theme_bw(40) + ylab("Abundance") + xlab("Year") + ggtitle("B)") + theme(legend.position = "none") +
  geom_boxplot(data=comm_data[comm_data$treatment!="P",], size=1.75, outlier.size=4, aes(x=year, y=abundance, colour=treatment)) +
  geom_boxplot(data=comm_data[comm_data$treatment=="P",], size=1.75, outlier.size=4, aes(x=year, y=abundance, colour=treatment)) +
  scale_color_manual(breaks = c("N","L","M","H","P"), values=c("red2", "dodgerblue","chartreuse3","darkorchid3","darkred"))

#making inset for abundance panel
inset <- ggplot() + theme_bw(40) + ylab("Abundance") + xlab("Year") + theme(legend.position = "none", axis.title.x=element_blank(), axis.title.y=element_blank()) +
  geom_boxplot(data=comm_data[comm_data$treatment!="P",], size=1, outlier.size=2.5, aes(x=year, y=abundance, colour=treatment)) +
  scale_color_manual(breaks = c("N","L","M","H"), values=c("red2", "dodgerblue","chartreuse3","darkorchid3"))

ggsave("./Figure2_inset.png", inset, height = 5, width = 7.5)
